...
while True:
    ...
    rects = detector.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, minSize=(30, 30), flags=cv2.CASCADE_SCALE_IMAGE)
    ...
    for (fX, fY, fW, fH) in rects:
        ...
        if label != prev_label:
            # The label has changed, you can do something here
            if label == 'Smiling':
                topic = "esp32/output"
                message = "1"
                ...
                message = "0"
                publish_to_topic(topic, message)
            print("Label has changed from '{}' to '{}'".format(prev_label, label))
        prev_label = label
